...
par_function <- function(y) {
  name<-paste(y)
  out <- tryCatch({
	  glmulti(formula(paste(y, "scale(classe_drainage)+scale(D_index)+scale(defoliation)+scale(epid_age)+scale(light)+scale(dist_from_river)", sep="~")), data=tree_df, random="+(1|type)", fitfunc=lmer.wrap, intercept=TRUE, confsetsize = 10, level=1)
